  ...
  if (!client) {
    return (
      <div className="container mx-auto px-4 py-6">
        <div className={`${darkMode ? 'bg-red-900/20 border-red-700 text-red-400' : 'bg-red-100 border-red-400 text-red-700'} border px-4 py-3 rounded transition-colors duration-200`}>
          Client not found
        </div>
        <div className="mt-4">
          <Link to="/" className={`${darkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-800'} transition-colors duration-200`}>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start">
            ...
            <div className="mt-6 sm:mt-0 flex space-x-3">
              ...
              >
                {isProcessing ? (
                  <>
                    <svg className={`animate-spin -ml-1 mr-2 h-4 w-4 ${darkMode ? 'text-gray-400' : 'text-white'}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              ...
              <button
                ...
                className={`inline-flex items-center py-2.5 px-4 rounded-lg text-sm font-medium transition-all duration-200 ${
                  client.coffees_purchased < redemptionThreshold || isProcessing
                    ? `${darkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-500'} cursor-not-allowed`
